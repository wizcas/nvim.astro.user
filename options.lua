...
local options = {
    opt = {
        ...
        textwidth = 80, -- the maximum column width of a single line
        timeoutlen = 200, -- time to wait for a mapped sequence to complete (in milliseconds)
        undofile = true, -- enable persistent undo
        updatetime = 300, -- faster completion (4000ms default)
        autoread = true, -- automatically reload changed files
        guifont = "CaskaydiaCove Nerd Font,CaskaydiaCove NF,Fira Code:h16", -- fonts for GUI clients
    },
    g = {
...
return function(local_vim)
    ...
    if vim.g.neovide then
        ...
        for k,v in pairs(neovide_opts) do vim.g[k] = v end
    end
